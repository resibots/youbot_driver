...
int configure_port(int fd) // configure the port
{
    ...
    port_settings.c_lflag &= ~(ICANON | ECHO | ECHOE | ISIG); // Choosing Raw
    // Input
...
void getIPAdress(std::string lanname, std::string wlanname, std::string& lanip,
    std::string& wlanip)
{
...
int main(int argc, char* argv[])
{
    try {
        if (argc != 4) {
            std::cout << "invalid arguments \n ./displayipaddress 'serial port' "
                         "'ethernet' 'wlan' \n./displayipaddress /dev/ttyACM0 eth1 "
                         "wlan0" << std::endl;
            return 0;
        }
        ...
        while (true) {
            ...
            //   std::cout << "powersupply: " << getVoltage(fd, powersupply) <<
            //   std::endl;
        }
