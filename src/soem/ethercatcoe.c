 ...
 */
void ecx_SDOerror(ecx_contextt* context, uint16 Slave, uint16 Index,
    uint8 SubIdx, int32 AbortCode)
{
    ec_errort Ec;
 ...
 * @param[in]  AbortCode  = Abortcode, see EtherCAT documentation for list
 */
static void ecx_SDOinfoerror(ecx_contextt* context, uint16 Slave, uint16 Index,
    uint8 SubIdx, int32 AbortCode)
{
    ec_errort Ec;
 ...
 *4bytes
 * then a "expedited" response is returned, otherwise a "normal" response. If a
 *"normal"
 * response is larger than the mailbox size then the response is segmented. The
 *function
 ...
 * @param[in]  CA         = FALSE = single subindex. TRUE = Complete Access, all
 *subindexes read.
    ...
    uint8 subindex, boolean CA, int* psize, void* p, int timeout)
{
    ...
    SDOp->CANOpen = htoes(
        0x000 + (ECT_COES_SDOREQ
                    << 12)); /* number 9bits service upper 4 bits (SDO request) */
    if (CA) {
                        ...
                        if (Framedatasize < SDOlen) /* transfer in segments? */
                        {
                            ...
                            *psize = Framedatasize;
                            ...
                            while (NotLast) /* segmented transfer */
                            {
                                ...
                                context->slavelist[slave].mbx_cnt = cnt;
                                SDOp->MbxHeader.mbxtype = ECT_MBXT_COE + (cnt << 4); /* CoE */
                                SDOp->CANOpen = htoes(0x000 + (ECT_COES_SDOREQ << 12)); /* number 9bits
                                                               service upper 4
                                                               bits (SDO
                                                               request) */
                                ...
                                wkc = ecx_mbxsend(context, slave, (ec_mbxbuft*)&MbxOut,
                                    EC_TIMEOUTTXM);
                                /* is mailbox transfered to slave ? */
                                if (wkc > 0) {
                                    ec_clearmbx(&MbxIn);
                                    /* read slave response */
                                    wkc = ecx_mbxreceive(context, slave, (ec_mbxbuft*)&MbxIn,
                                        timeout);
                                    /* has slave responded ? */
                                    if (wkc > 0) {
                                        /* slave response should be CoE, SDO response */
                                        if ((((aSDOp->MbxHeader.mbxtype & 0x0f) == ECT_MBXT_COE) && ((etohs(aSDOp->CANOpen) >> 12) == ECT_COES_SDORES) && ((aSDOp->Command & 0xe0) == 0x00))) {
                                            /* calculate mailbox transfer size */
                                            Framedatasize = etohs(aSDOp->MbxHeader.length) - 3;
                                            if ((aSDOp->Command & 0x01) > 0) { /* last segment */
                                                NotLast = FALSE;
                                                if (Framedatasize == 7)
 ...
 * small data, then a "expedited" transfer is used. If the parameter is larger
 *than
    ...
    if ((psize <= 4) && !CA) {
        ...
        if (wkc > 0) {
            ...
            if (wkc > 0) {
                ...
                if (((aSDOp->MbxHeader.mbxtype & 0x0f) == ECT_MBXT_COE) && ((etohs(aSDOp->CANOpen) >> 12) == ECT_COES_SDORES) && (aSDOp->Index == SDOp->Index) && (aSDOp->SubIndex == SDOp->SubIndex)) {
                    /* all OK */
                }
        ...
        SDOp->CANOpen = htoes(0x000 + (ECT_COES_SDOREQ << 12)); /* number 9bits service upper 4 bits */
        if (CA) {
            SDOp->Command = ECT_SDO_DOWN_INIT_CA; /* Complete Access, normal SDO init download
                                   transfer */
        }
        else {
                ...
                if (((aSDOp->MbxHeader.mbxtype & 0x0f) == ECT_MBXT_COE) && ((etohs(aSDOp->CANOpen) >> 12) == ECT_COES_SDORES) && (aSDOp->Index == SDOp->Index) && (aSDOp->SubIndex == SDOp->SubIndex)) {
                    ...
                    while (NotLast) {
                        ...
                        if (!NotLast && (framedatasize < 7)) {
                            ...
                            SDOp->Command = 0x01 + ((7 - framedatasize)
                                                       << 1); /* last segment reduced octets */
                        }
                        else {
                        ...
                        if (wkc > 0) {
                            ...
                            wkc = ecx_mbxreceive(context, Slave, (ec_mbxbuft*)&MbxIn, Timeout);
                            if (wkc > 0) {
                                if (((aSDOp->MbxHeader.mbxtype & 0x0f) == ECT_MBXT_COE) && ((etohs(aSDOp->CANOpen) >> 12) == ECT_COES_SDORES) && ((aSDOp->Command & 0xe0) == 0x20)) {
 ...
 * @param[in,out] psize      = Size in bytes of PDO buffer, returns bytes read
 *from PDO.
 * @param[out] p             = Pointer to PDO buffer
 * @param[in]  timeout       = Timeout in us, standard is EC_TIMEOUTRXM
 * @return Workcounter from last slave response
    ...
    int* psize, void* p, int timeout)
{
    ...
    if (wkc > 0) {
        ...
        if (wkc > 0) /* succeeded to read slave response ? */
        {
            ...
            if (((aSDOp->MbxHeader.mbxtype & 0x0f) == ECT_MBXT_COE) && ((etohs(aSDOp->CANOpen) >> 12) == ECT_COES_TXPDO)) {
                /* TxPDO response */
                ...
                /* parameter buffer too small */
                    ...
                    wkc = 0;
                    ecx_packeterror(context, slave, 0, 0,
                        3); /*  data container too small for type */
                }
            }
            /* other slave response */
...
int ecx_readPDOassign(ecx_contextt* context, uint16 Slave, uint16 PDOassign)
{
    ...
    wkc = ecx_SDOread(context, Slave, PDOassign, 0x00, FALSE, &rdl, &rdat,
        EC_TIMEOUTRXM);
    rdat = etohs(rdat);
    ...
    if ((wkc > 0) && (rdat > 0)) {
        ...
        for (idxloop = 1; idxloop <= nidx; idxloop++) {
            ...
            rdat = 0;
            /* read PDO assign */
            wkc = ecx_SDOread(context, Slave, PDOassign, (uint8)idxloop, FALSE, &rdl,
                &rdat, EC_TIMEOUTRXM);
            ...
            if (idx > 0) {
                ...
                wkc = ecx_SDOread(context, Slave, idx, 0x00, FALSE, &rdl, &subcnt,
                    EC_TIMEOUTRXM);
                subidx = subcnt;
                /* for each subindex */
                for (subidxloop = 1; subidxloop <= subidx; subidxloop++) {
                    ...
                    wkc = ecx_SDOread(context, Slave, idx, (uint8)subidxloop, FALSE, &rdl,
                        &rdat2, EC_TIMEOUTRXM);
                    rdat2 = etohl(rdat2);
                    ...
                    if (LO_BYTE(rdat2) < 0xff) {
                        ...
                        //                  wkc = ec_readOEsingle(idx, (uint8)SubCount,
                        //                  pODlist, pOElist);
                        bsize += etohs(rdat);
                    }
                }
...
int ecx_readPDOassignCA(ecx_contextt* context, uint16 Slave, uint16 PDOassign)
{
    ...
    wkc = ecx_SDOread(context, Slave, PDOassign, 0x00, TRUE, &rdl,
        context->PDOassign, EC_TIMEOUTRXM);
    /* positive result from slave ? */
 ...
 */
int ecx_readPDOmap(ecx_contextt* context, uint16 Slave, int* Osize,
    int* Isize)
{
    int wkc, rdl;
    int retVal = 0;
    ...
    *Osize = 0;
    ...
    wkc = ecx_SDOread(context, Slave, ECT_SDO_SMCOMMTYPE, 0x00, FALSE, &rdl, &nSM,
        EC_TIMEOUTRXM);
    /* positive result from slave ? */
    if ((wkc > 0) && (nSM > 2)) {
     ...
     * configured */
        ...
        for (iSM = 2; iSM <= nSM; iSM++) {
            ...
            wkc = ecx_SDOread(context, Slave, ECT_SDO_SMCOMMTYPE, iSM + 1, FALSE,
                &rdl, &tSM, EC_TIMEOUTRXM);
            if (wkc > 0) {
                ...
                if (tSM == 0) {
                    context->slavelist[Slave].SM[iSM].SMflags = htoel(etohl(context->slavelist[Slave].SM[iSM].SMflags) & EC_SMENABLEMASK);
                }
                if ((tSM == 3) || (tSM == 4)) {
                    /* read the assign PDO */
 ...
 * @return >0 if mapping succesful.
 */
int ecx_readPDOmapCA(ecx_contextt* context, uint16 Slave, int* Osize,
    int* Isize)
{
    int wkc, rdl;
    int retVal = 0;
    ...
    *Osize = 0;
    ...
    wkc = ecx_SDOread(context, Slave, ECT_SDO_SMCOMMTYPE, 0x00, TRUE, &rdl,
        context->SMcommtype, EC_TIMEOUTRXM);
    /* positive result from slave ? */
     ...
     * configured */
        ...
        for (iSM = 2; iSM <= nSM; iSM++) {
            ...
            if ((iSM == 2) && (tSM == 2)) // SM2 has type 2 == mailbox out, this is a bug in the slave!
            {
                SMt_bug_add = 1; // try to correct, this works if the types are 0 1 2 3
            ...
            /* check if SM is unused -> clear enable flag */
            if (tSM == 0) {
                context->slavelist[Slave].SM[iSM].SMflags = htoel(
                    etohl(context->slavelist[Slave].SM[iSM].SMflags) & EC_SMENABLEMASK);
            }
            if ((tSM == 3) || (tSM == 4)) {
                /* read the assign PDO */
...
int ecx_readODlist(ecx_contextt* context, uint16 Slave, ec_ODlistt* pODlist)
{
    ...
    SDOp->MbxHeader.mbxtype = ECT_MBXT_COE + (cnt << 4); /* CoE */
    SDOp->CANOpen = htoes(
        0x000 + (ECT_COES_SDOINFO << 12)); /* number 9bits service upper 4 bits */
    ...
    if (wkc > 0) {
        ...
        do {
            ...
            if (wkc > 0) {
                ...
                if (((aSDOp->MbxHeader.mbxtype & 0x0f) == ECT_MBXT_COE) && ((aSDOp->Opcode & 0x7f) == ECT_GET_ODLIST_RES)) {
                    if (First) {
                    ...
                    if ((sp + n) > EC_MAXODLIST) {
                        ...
                        ecx_SDOinfoerror(
                            context, Slave, 0, 0,
                            0xf000000); /* Too many entries for master buffer */
                        stop = TRUE;
                    }
    ...
    if (wkc > 0) {
        ...
        if (wkc > 0) {
            ...
            /* got unexpected response from slave */
                ...
                if (((aSDOp->Opcode & 0x7f) == ECT_SDOINFO_ERROR)) /* SDO info error received */
                {
                    ecx_SDOinfoerror(context, Slave, pODlist->Index[Item], 0,
                        etohl(aSDOp->ldata[0]));
                }
                else {
                    ecx_packeterror(context, Slave, pODlist->Index[Item], 0,
                        1); /* Unexpected frame returned */
                }
                wkc = 0;
            }
 ...
 * @return Workcounter of slave response.
 */
int ecx_readOEsingle(ecx_contextt* context, uint16 Item, uint8 SubI,
    ec_ODlistt* pODlist, ec_OElistt* pOElist)
{
    ec_SDOservicet* SDOp, *aSDOp;
 ...
 * @return Workcounter of slave response.
 */
int ecx_readOE(ecx_contextt* context, uint16 Item, ec_ODlistt* pODlist,
    ec_OElistt* pOElist)
{
    uint16 SubCount;
    int wkc;
...
int ec_SDOread(uint16 slave, uint16 index, uint8 subindex, boolean CA,
    int* psize, void* p, int timeout)
{
    return ecx_SDOread(&ecx_context, slave, index, subindex, CA, psize, p,
        timeout);
}
...
int ec_SDOwrite(uint16 Slave, uint16 Index, uint8 SubIndex, boolean CA,
    int psize, void* p, int Timeout)
{
    return ecx_SDOwrite(&ecx_context, Slave, Index, SubIndex, CA, psize, p,
        Timeout);
}
...
int ec_TxPDO(uint16 slave, uint16 TxPDOnumber, int* psize, void* p,
    int timeout)
{
...
int ec_readOEsingle(uint16 Item, uint8 SubI, ec_ODlistt* pODlist,
    ec_OElistt* pOElist)
{
