 ...
 *
 * @param[in] adapter = Struct holding adapter name, description and pointer to
 *next.
 */
void ec_free_adapters(ec_adaptert* adapter) { oshw_free_adapters(adapter); }
 ...
 */
void ecx_packeterror(ecx_contextt* context, uint16 Slave, uint16 Index,
    uint8 SubIdx, uint16 ErrorCode)
{
    ec_errort Ec;
 ...
 * @param[in]  w2
 */
static void ecx_mbxemergencyerror(ecx_contextt* context, uint16 Slave,
    uint16 ErrorCode, uint16 ErrorReg, uint8 b1,
    uint16 w1, uint16 w2)
{
    ec_errort Ec;
    ...
    char* ifname, char* if2name)
{
    ...
    zbuf = 0;
    ecx_setupdatagram(context->port, &(context->port->txbuf2), EC_CMD_BRD, 0,
        0x0000, 0x0000, 2, &zbuf);
 ...
 * @param[in]  context        = context struct
 */
void ecx_close(ecx_contextt* context) { ecx_closenic(context->port); };
 ...
 *  @return byte address of section at section length entry, if not available
 * then 0
 */
int16 ecx_siifind(ecx_contextt* context, uint16 slave, uint16 cat)
{
    ...
    if (eectl) {
        ecx_eeprom2pdi(
            context, slave); /* if eeprom control was previously pdi then restore */
    }
...
void ecx_siistring(ecx_contextt* context, char* str, uint16 slave, uint16 Sn)
{
    ...
    if (a > 0) {
        ...
        n = ecx_siigetbyte(context, slave,
            ba++); /* read number of strings in section */
 ...
 *  @return >0 if OK
 */
uint16 ecx_siiSMnext(ecx_contextt* context, uint16 slave, ec_eepromSMt* SM,
    uint16 n)
{
    uint16 a;
    uint16 retVal = 0;
 ...
 */
int ecx_siiPDO(ecx_contextt* context, uint16 slave, ec_eepromPDOt* PDO,
    uint8 t)
{
...
int ecx_readstate(ecx_contextt* context)
{
    ...
    for (slave = 1; slave <= *(context->slavecount); slave++) {
        ...
        ecx_FPRD(context->port, configadr, ECT_REG_ALSTAT, sizeof(slstat), &slstat,
            EC_TIMEOUTRET3);
...
int ecx_writestate(ecx_contextt* context, uint16 slave)
{
    ...
    if (slave == 0) {
        ...
        ecx_BWR(context->port, 0, ECT_REG_ALCTL, sizeof(slstate), &slstate,
            EC_TIMEOUTRET3); /* write slave status */
    }
    else {
        configadr = context->slavelist[slave].configadr;

        ecx_FPWRw(context->port, configadr, ECT_REG_ALCTL,
            htoes(context->slavelist[slave].state),
            EC_TIMEOUTRET3); /* write slave status */
    }
    return 0;
}
 ...
 */
uint16 ecx_statecheck(ecx_contextt* context, uint16 slave, uint16 reqstate,
    int timeout)
{
    ...
    do {
        if (slave < 1) {
            rval = 0;
            ecx_BRD(context->port, 0, ECT_REG_ALSTAT, sizeof(rval), &rval,
                EC_TIMEOUTRET);
            rval = etohs(rval);
        }
        else {
            slstat.alstatus = 0;
            slstat.alstatuscode = 0;
            ecx_FPRD(context->port, configadr, ECT_REG_ALSTAT, sizeof(slstat),
                &slstat, EC_TIMEOUTRET);
            rval = etohs(slstat.alstatus);
            context->slavelist[slave].ALstatuscode = etohs(slstat.alstatuscode);
        }
 ...
 */
void ec_clearmbx(ec_mbxbuft* Mbx) { memset(*Mbx, 0x00, EC_MAXMBX); }
...
int ecx_mbxempty(ecx_contextt* context, uint16 slave, int timeout)
{
    ...
    configadr = context->slavelist[slave].configadr;
    do {
        wkc = ecx_FPRD(context->port, configadr, ECT_REG_SM0STAT, sizeof(SMstat),
            &SMstat, EC_TIMEOUTRET);
        SMstat = etohs(SMstat);
 ...
 */
int ecx_mbxsend(ecx_contextt* context, uint16 slave, ec_mbxbuft* mbx,
    int timeout)
{
 ...
 * @return Work counter (>0 is success)
 */
int ecx_mbxreceive(ecx_contextt* context, uint16 slave, ec_mbxbuft* mbx,
    int timeout)
{
    ...
    if (mbxl > 0) {
        ...
        {
            wkc = ecx_FPRD(context->port, configadr, ECT_REG_SM1STAT, sizeof(SMstat),
                &SMstat, EC_TIMEOUTRET);
            SMstat = etohs(SMstat);
            if (((SMstat & 0x08) == 0) && (timeout > EC_LOCALDELAY)) {
                osal_usleep(EC_LOCALDELAY);
        ...
        if ((wkc > 0) && ((SMstat & 0x08) > 0)) /* read mailbox available ? */
        {
            ...
            do {
                wkc = ecx_FPRD(context->port, configadr, mbxro, mbxl, mbx,
                    EC_TIMEOUTRET); /* get mailbox */

                //          // EtherCAT slaves of KUKA youBot don't send standard
                //          mailbox header as reply,
                //          // so don't check for error codes here
                //
                ...
                //               wkc = 0; /* prevent emergency to cascade up, it is
                //               already handled. */
                //            }
                //            else if ((wkc > 0) && ((mbxh->mbxtype & 0x0f) == 0x03)) /*
                //            CoE response? */
                //            {
                //               EMp = (ec_emcyt *)mbx;
                //               if ((etohs(EMp->CANOpen) >> 12) == 0x01) /* Emergency
                //               request? */
                //               {
                //                  ecx_mbxemergencyerror(context, slave,
                //                  etohs(EMp->ErrorCode), EMp->ErrorReg,
                ...
                //            else
                {
                    if (wkc <= 0) /* read mailbox lost */
                    {
                        ...
                        SMstat = htoes(SMstat);
                        wkc2 = ecx_FPWR(context->port, configadr, ECT_REG_SM1STAT,
                            sizeof(SMstat), &SMstat, EC_TIMEOUTRET);
                        SMstat = etohs(SMstat);
                        do /* wait for toggle ack */
                        {
                            wkc2 = ecx_FPRD(context->port, configadr, ECT_REG_SM1CONTR,
                                sizeof(SMcontr), &SMcontr, EC_TIMEOUTRET);
                        } while (((wkc2 <= 0) || ((SMcontr & 0x02) != (HI_BYTE(SMstat) & 0x02))) && (osal_timer_is_expired(&timer) == FALSE));
                        do /* wait for read mailbox available */
                        {
                            wkc2 = ecx_FPRD(context->port, configadr, ECT_REG_SM1STAT,
                                sizeof(SMstat), &SMstat, EC_TIMEOUTRET);
                            SMstat = etohs(SMstat);
                            if (((SMstat & 0x08) == 0) && (timeout > EC_LOCALDELAY)) {
                                osal_usleep(EC_LOCALDELAY);
 ...
 */
uint32 ecx_readeeprom(ecx_contextt* context, uint16 slave, uint16 eeproma,
    int timeout)
{
    uint16 configadr;
 ...
 * @return >0 if OK
 */
int ecx_writeeeprom(ecx_contextt* context, uint16 slave, uint16 eeproma,
    uint16 data, int timeout)
{
    uint16 configadr;
...
int ecx_eeprom2master(ecx_contextt* context, uint16 slave)
{
    ...
    if (context->slavelist[slave].eep_pdi) {
        ...
        eepctl = 2;
        do {
            wkc = ecx_FPWR(context->port, configadr, ECT_REG_EEPCFG, sizeof(eepctl),
                &eepctl, EC_TIMEOUTRET); /* force Eeprom from PDI */
...
int ecx_eeprom2pdi(ecx_contextt* context, uint16 slave)
{
    ...
    if (!context->slavelist[slave].eep_pdi) {
        ...
        do {
            wkc = ecx_FPWR(context->port, configadr, ECT_REG_EEPCFG, sizeof(eepctl),
                &eepctl, EC_TIMEOUTRET); /* set Eeprom to PDI */
        } while ((wkc <= 0) && (cnt++ < EC_DEFAULTRETRIES));
...
uint16 ecx_eeprom_waitnotbusyAP(ecx_contextt* context, uint16 aiadr,
    uint16* estat, int timeout)
{
    ...
    do {
        ...
        wkc = ecx_APRD(context->port, aiadr, ECT_REG_EEPSTAT, sizeof(*estat), estat,
            EC_TIMEOUTRET);
        *estat = etohs(*estat);
    } while (
        ((wkc <= 0) || ((*estat & EC_ESTAT_BUSY) > 0)) && (osal_timer_is_expired(&timer) == FALSE)); /* wait for eeprom ready */
 ...
 */
uint64 ecx_readeepromAP(ecx_contextt* context, uint16 aiadr, uint16 eeproma,
    int timeout)
{
    uint16 estat;
    uint32 edat32;
    ...
    if (ecx_eeprom_waitnotbusyAP(context, aiadr, &estat, timeout)) {
        if (estat & EC_ESTAT_EMASK) /* error bits are set */
        {
            ...
            wkc = ecx_APWR(context->port, aiadr, ECT_REG_EEPCTL, sizeof(estat),
                &estat, EC_TIMEOUTRET3);
        }
        ...
        do {
            ...
            cnt = 0;
            do {
                wkc = ecx_APWR(context->port, aiadr, ECT_REG_EEPCTL, sizeof(ed), &ed,
                    EC_TIMEOUTRET);
            } while ((wkc <= 0) && (cnt++ < EC_DEFAULTRETRIES));
            if (wkc) {
                osal_usleep(EC_LOCALDELAY);
                ...
                if (ecx_eeprom_waitnotbusyAP(context, aiadr, &estat, timeout)) {
                    if (estat & EC_ESTAT_NACK) {
                        ...
                        if (estat & EC_ESTAT_R64) {
                            cnt = 0;
                            do {
                                wkc = ecx_APRD(context->port, aiadr, ECT_REG_EEPDAT,
                                    sizeof(edat64), &edat64, EC_TIMEOUTRET);
                            } while ((wkc <= 0) && (cnt++ < EC_DEFAULTRETRIES));
                        }
                        else {
                            cnt = 0;
                            do {
                                wkc = ecx_APRD(context->port, aiadr, ECT_REG_EEPDAT,
                                    sizeof(edat32), &edat32, EC_TIMEOUTRET);
                            } while ((wkc <= 0) && (cnt++ < EC_DEFAULTRETRIES));
 ...
 * @return >0 if OK
 */
int ecx_writeeepromAP(ecx_contextt* context, uint16 aiadr, uint16 eeproma,
    uint16 data, int timeout)
{
    uint16 estat;
    ec_eepromt ed;
    ...
    if (ecx_eeprom_waitnotbusyAP(context, aiadr, &estat, timeout)) {
        ...
        do {
            cnt = 0;
            do {
                wkc = ecx_APWR(context->port, aiadr, ECT_REG_EEPDAT, sizeof(data),
                    &data, EC_TIMEOUTRET);
            } while ((wkc <= 0) && (cnt++ < EC_DEFAULTRETRIES));
...
uint16 ecx_eeprom_waitnotbusyFP(ecx_contextt* context, uint16 configadr,
    uint16* estat, int timeout)
{
    int wkc, cnt = 0, retval = 0;
    osal_timert timer;
    ...
    do {
        ...
        wkc = ecx_FPRD(context->port, configadr, ECT_REG_EEPSTAT, sizeof(*estat),
            estat, EC_TIMEOUTRET);
        *estat = etohs(*estat);
    } while (
        ((wkc <= 0) || ((*estat & EC_ESTAT_BUSY) > 0)) && (osal_timer_is_expired(&timer) == FALSE)); /* wait for eeprom ready */
 ...
 * @return EEPROM data 64bit or 32bit
 */
uint64 ecx_readeepromFP(ecx_contextt* context, uint16 configadr, uint16 eeproma,
    int timeout)
{
    uint16 estat;
    uint32 edat32;
    ...
    if (ecx_eeprom_waitnotbusyFP(context, configadr, &estat, timeout)) {
        ...
        {
            estat = htoes(EC_ECMD_NOP); /* clear error bits */
            wkc = ecx_FPWR(context->port, configadr, ECT_REG_EEPCTL, sizeof(estat),
                &estat, EC_TIMEOUTRET3);
        }
        ...
        do {
            ...
            cnt = 0;
            do {
                wkc = ecx_FPWR(context->port, configadr, ECT_REG_EEPCTL, sizeof(ed),
                    &ed, EC_TIMEOUTRET);
            } while ((wkc <= 0) && (cnt++ < EC_DEFAULTRETRIES));
            if (wkc) {
                osal_usleep(EC_LOCALDELAY);
                ...
                if (ecx_eeprom_waitnotbusyFP(context, configadr, &estat, timeout)) {
                    if (estat & EC_ESTAT_NACK) {
                        ...
                        if (estat & EC_ESTAT_R64) {
                            cnt = 0;
                            do {
                                wkc = ecx_FPRD(context->port, configadr, ECT_REG_EEPDAT,
                                    sizeof(edat64), &edat64, EC_TIMEOUTRET);
                            } while ((wkc <= 0) && (cnt++ < EC_DEFAULTRETRIES));
                        }
                        else {
 ...
 * @return >0 if OK
 */
int ecx_writeeepromFP(ecx_contextt* context, uint16 configadr, uint16 eeproma,
    uint16 data, int timeout)
{
    uint16 estat;
    ec_eepromt ed;
    ...
    if (ecx_eeprom_waitnotbusyFP(context, configadr, &estat, timeout)) {
        ...
        do {
            cnt = 0;
            do {
                wkc = ecx_FPWR(context->port, configadr, ECT_REG_EEPDAT, sizeof(data),
                    &data, EC_TIMEOUTRET);
            } while ((wkc <= 0) && (cnt++ < EC_DEFAULTRETRIES));
            ed.comm = EC_ECMD_WRITE;
            ed.addr = eeproma;
...
void ecx_readeeprom1(ecx_contextt* context, uint16 slave, uint16 eeproma)
{
    ...
    if (ecx_eeprom_waitnotbusyFP(context, configadr, &estat, EC_TIMEOUTEEP)) {
        ...
        ed.d2 = 0x0000;
        do {
            wkc = ecx_FPWR(context->port, configadr, ECT_REG_EEPCTL, sizeof(ed), &ed,
                EC_TIMEOUTRET);
        } while ((wkc <= 0) && (cnt++ < EC_DEFAULTRETRIES));
    }
}
...
uint32 ecx_readeeprom2(ecx_contextt* context, uint16 slave, int timeout)
{
    ...
    if (ecx_eeprom_waitnotbusyFP(context, configadr, &estat, timeout)) {
        do {
            wkc = ecx_FPRD(context->port, configadr, ECT_REG_EEPDAT, sizeof(edat),
                &edat, EC_TIMEOUTRET);
        } while ((wkc <= 0) && (cnt++ < EC_DEFAULTRETRIES));
    }
 ...
 */
static void ecx_pushindex(ecx_contextt* context, uint8 idx, void* data,
    uint16 length)
{
    ...
    if (length) {
        ...
        if (context->grouplist[group].blockLRW) {
            ...
            if (context->grouplist[group].Ibytes) {
                ...
                do {
                    ...
                    w1 = LO_WORD(LogAdr);
                    w2 = HI_WORD(LogAdr);
                    ecx_setupdatagram(context->port, &(context->port->txbuf[idx]),
                        EC_CMD_LRD, idx, w1, w2, sublength, data);
                    if (first) {
                        context->DCl = sublength;
                        /* FPRMW in second datagram */
                        context->DCtO = ecx_adddatagram(
                            context->port, &(context->port->txbuf[idx]), EC_CMD_FRMW, idx,
                            FALSE,
            ...
            if (context->grouplist[group].Obytes) {
                ...
                do {
                    ...
                    w2 = HI_WORD(LogAdr);
                    ecx_setupdatagram(context->port, &(context->port->txbuf[idx]),
                        EC_CMD_LWR, idx, w1, w2, sublength, data);
                    if (first) {
                        context->DCl = sublength;
                        /* FPRMW in second datagram */
                        context->DCtO = ecx_adddatagram(
        ...
        /* LRW can be used */
            ...
            do {
                ...
                w2 = HI_WORD(LogAdr);
                ecx_setupdatagram(context->port, &(context->port->txbuf[idx]),
                    EC_CMD_LRW, idx, w1, w2, sublength, data);
                if (first) {
                    context->DCl = sublength;
                    /* FPRMW in second datagram */
                    context->DCtO = ecx_adddatagram(
 ...
 * Received datagrams are recombined with the processdata with help from the
 * stack.
 * If a datagram contains input processdata it copies it to the processdata
 * structure.
 * @param[in]  context        = context struct
 * @param[in]  group          = group number
 * @param[in]  timeout        = Timeout in us.
 * @return Work counter.
 */
int ecx_receive_processdata_group(ecx_contextt* context, uint8 group,
    int timeout)
{
    int pos, idx;
    int wkc = 0, wkc2;
    ...
    while (pos >= 0) {
        ...
        if (wkc2 > EC_NOFRAME) {
            if ((context->port->rxbuf[idx][EC_CMDOFFSET] == EC_CMD_LRD) || (context->port->rxbuf[idx][EC_CMDOFFSET] == EC_CMD_LRW)) {
                if (first) {
                    memcpy(context->idxstack->data[pos],
                        &(context->port->rxbuf[idx][EC_HEADERSIZE]), context->DCl);
                    memcpy(&le_wkc,
                        &(context->port->rxbuf[idx][EC_HEADERSIZE + context->DCl]),
                        EC_WKCSIZE);
                    wkc = etohs(le_wkc);
                    memcpy(&le_DCtime, &(context->port->rxbuf[idx][context->DCtO]),
                        sizeof(le_DCtime));
                    *(context->DCtime) = etohll(le_DCtime);
                    ...
                    memcpy(context->idxstack->data[pos],
                        &(context->port->rxbuf[idx][EC_HEADERSIZE]),
...
boolean ec_poperror(ec_errort* Ec) { return ecx_poperror(&ecx_context, Ec); }

boolean ec_iserror(void) { return ecx_iserror(&ecx_context); }

void ec_packeterror(uint16 Slave, uint16 Index, uint8 SubIdx,
    uint16 ErrorCode)
{
...
static void ec_mbxemergencyerror(uint16 Slave, uint16 ErrorCode,
    uint16 ErrorReg, uint8 b1, uint16 w1,
    uint16 w2)
{
    ecx_mbxemergencyerror(&ecx_context, Slave, ErrorCode, ErrorReg, b1, w1, w2);
}

int ec_init(char* ifname) { return ecx_init(&ecx_context, ifname); }
...
int ec_readstate(void) { return ecx_readstate(&ecx_context); }

int ec_writestate(uint16 slave) { return ecx_writestate(&ecx_context, slave); }
...
int ec_eeprom2pdi(uint16 slave) { return ecx_eeprom2pdi(&ecx_context, slave); }
...
int ec_writeeepromFP(uint16 configadr, uint16 eeproma, uint16 data,
    int timeout)
{
...
int ec_send_processdata(void) { return ec_send_processdata_group(0); }
