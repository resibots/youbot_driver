 ...
 */
void ecx_SoEerror(ecx_contextt* context, uint16 Slave, uint16 idn,
    uint16 Error)
{
    ec_errort Ec;
 ...
 * @param[in]  elementflags  = Flags to select what properties of IDN are to be
 *transfered.
 * @param[in]  idn           = IDN.
 ...
 */
int ecx_SoEread(ecx_contextt* context, uint16 slave, uint8 driveNo,
    uint8 elementflags, uint16 idn, int* psize, void* p,
    int timeout)
{
    ec_SoEt* SoEp, *aSoEp;
    ...
    if (wkc > 0) /* succeeded to place mailbox in slave ? */
    {
        while (NotLast) {
            ...
            if (wkc > 0) /* succeeded to read slave response ? */
            {
                ...
                if (((aSoEp->MbxHeader.mbxtype & 0x0f) == ECT_MBXT_SOE) && (aSoEp->opCode == ECT_SOE_READRES) && (aSoEp->error == 0) && (aSoEp->driveNo == driveNo) && (aSoEp->elementflags == elementflags)) {
                ...
                /* other slave response */
                    ...
                    NotLast = FALSE;
                    if (((aSoEp->MbxHeader.mbxtype & 0x0f) == ECT_MBXT_SOE) && (aSoEp->opCode == ECT_SOE_READRES) && (aSoEp->error == 1)) {
                        mp = (uint8*)&MbxIn + (etohs(aSoEp->MbxHeader.length) + sizeof(ec_mbxheadert) - sizeof(uint16));
                        errorcode = (uint16*)mp;
                        ecx_SoEerror(context, slave, idn, *errorcode);
                    }
                    else {
                        ecx_packeterror(context, slave, idn, 0,
 ...
 * @return Workcounter from last slave response
 */
int ecx_SoEwrite(ecx_contextt* context, uint16 slave, uint8 driveNo,
    uint8 elementflags, uint16 idn, int psize, void* p,
    int timeout)
{
    ec_SoEt* SoEp, *aSoEp;
        ...
        if (wkc > 0) /* succeeded to place mailbox in slave ? */
        {
            if (!NotLast || !ecx_mbxempty(context, slave, timeout)) {
                ...
                if (wkc > 0) /* succeeded to read slave response ? */
                {
                    ...
                    if (((aSoEp->MbxHeader.mbxtype & 0x0f) == ECT_MBXT_SOE) && (aSoEp->opCode == ECT_SOE_WRITERES) && (aSoEp->error == 0) && (aSoEp->driveNo == driveNo) && (aSoEp->elementflags == elementflags)) {
                        /* SoE write succeeded */
                    }
                    /* other slave response */
                    else {
 ...
 */
int ecx_readIDNmap(ecx_contextt* context, uint16 slave, int* Osize,
    int* Isize)
{
    int retVal = 0;
    int wkc;
    ...
    *Osize = 0;
    ...
    wkc = ecx_SoEread(context, slave, 0, EC_SOE_VALUE_B, EC_IDN_MDTCONFIG, &psize,
        &SoEmapping, EC_TIMEOUTRXM);
    if ((wkc > 0) && (psize >= 4) && ((entries = etohs(SoEmapping.currentlength) / 2) > 0) && (entries <= EC_SOE_MAXMAPPING)) {
        ...
        *Osize = 16;
        for (itemcount = 0; itemcount < entries; itemcount++) {
            ...
            wkc = ecx_SoEread(context, slave, 0, EC_SOE_ATTRIBUTE_B,
                SoEmapping.idn[itemcount], &psize, &SoEattribute,
                EC_TIMEOUTRXM);
    ...
    wkc = ecx_SoEread(context, slave, 0, EC_SOE_VALUE_B, EC_IDN_ATCONFIG, &psize,
        &SoEmapping, EC_TIMEOUTRXM);
    if ((wkc > 0) && (psize >= 4) && ((entries = etohs(SoEmapping.currentlength) / 2) > 0) && (entries <= EC_SOE_MAXMAPPING)) {
...
#ifdef EC_VER1
int ec_SoEread(uint16 slave, uint8 driveNo, uint8 elementflags, uint16 idn,
    int* psize, void* p, int timeout)
{
    return ecx_SoEread(&ecx_context, slave, driveNo, elementflags, idn, psize, p,
        timeout);
}

int ec_SoEwrite(uint16 slave, uint8 driveNo, uint8 elementflags, uint16 idn,
    int psize, void* p, int timeout)
{
    return ecx_SoEwrite(&ecx_context, slave, driveNo, elementflags, idn, psize, p,
        timeout);
}
