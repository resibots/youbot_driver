...
typedef struct ethernet_data {
    uint16_t
        length; /* When TX = frame data length in bytes, not including 'length'
...
static ethernet_data_t rxBuffer[ETH_RX_BUF_SIZE]
    __attribute__((section(".dma")));
static ethernet_data_t txBuffer[ETH_TX_BUF_SIZE]
    __attribute__((section(".dma")));
static volatile uint32_t rxStatusWord[ETH_RX_BUF_SIZE]
    __attribute__((section(".dma")));
static volatile uint32_t txStatusWord[ETH_TX_BUF_SIZE]
    __attribute__((section(".dma")));
static bfin_dma_descriptor_t rxDMADesc[ETH_RX_BUF_SIZE * 2]
    __attribute__((section(".dma")));
static bfin_dma_descriptor_t txDMADesc[ETH_TX_BUF_SIZE * 2]
    __attribute__((section(".dma")));
...
static void lw_emac_write_phy_reg(uint8_t phy_addr, uint8_t reg_addr,
    uint32_t data)
{
...
{
    pEth->addrlo = ethAddr[0] | ethAddr[1] << 8 | ethAddr[2] << 16 | ethAddr[3] << 24;
    pEth->addrhi = ethAddr[4] | ethAddr[5] << 8;
}
...
static uint8_t lw_emac_init_registers(uint8_t* ethAddr)
{
   ...
   * RXDWA = Pad incoming frame with 0x0000 as to make the data-part 32-bit
   * aligned
...
int bfin_EMAC_init(uint8_t* ethAddr)
{
    ...
    txDMADesc[0].config = DMA_CONFIG_DMA_EN | DMA_CONFIG_WDSIZE(DMA_WDSIZE_32BIT) | DMA_CONFIG_NDSIZE(5) | DMA_CONFIG_FLOW(DMA_FLOW_DESCRIPTOR_LIST_LARGE);

    txDMADesc[1].next = &txDMADesc[0];
    txDMADesc[1].start_addr = &txStatusWord[0];
    txDMADesc[1].config = DMA_CONFIG_DMA_EN | DMA_CONFIG_WNR | DMA_CONFIG_WDSIZE(DMA_WDSIZE_32BIT) | DMA_CONFIG_NDSIZE(0) | DMA_CONFIG_FLOW(DMA_FLOW_STOP);

    rxDMADesc[0].next = &rxDMADesc[1];
    rxDMADesc[0].start_addr = &rxBuffer[0];
    rxDMADesc[0].config = DMA_CONFIG_DMA_EN | DMA_CONFIG_WNR | DMA_CONFIG_WDSIZE(DMA_WDSIZE_32BIT) | DMA_CONFIG_NDSIZE(5) | DMA_CONFIG_FLOW(DMA_FLOW_DESCRIPTOR_LIST_LARGE);

    rxDMADesc[1].next = &rxDMADesc[2];
    rxDMADesc[1].start_addr = &rxStatusWord[0];
    rxDMADesc[1].config = DMA_CONFIG_DMA_EN | DMA_CONFIG_WNR | DMA_CONFIG_WDSIZE(DMA_WDSIZE_32BIT) | DMA_CONFIG_NDSIZE(5) | DMA_CONFIG_FLOW(DMA_FLOW_DESCRIPTOR_LIST_LARGE);

    rxDMADesc[2].next = &rxDMADesc[3];
    rxDMADesc[2].start_addr = &rxBuffer[1];
    rxDMADesc[2].config = DMA_CONFIG_DMA_EN | DMA_CONFIG_WNR | DMA_CONFIG_WDSIZE(DMA_WDSIZE_32BIT) | DMA_CONFIG_NDSIZE(5) | DMA_CONFIG_FLOW(DMA_FLOW_DESCRIPTOR_LIST_LARGE);

    rxDMADesc[3].next = &rxDMADesc[0];
    rxDMADesc[3].start_addr = &rxStatusWord[1];
    rxDMADesc[3].config = DMA_CONFIG_DMA_EN | DMA_CONFIG_WNR | DMA_CONFIG_WDSIZE(DMA_WDSIZE_32BIT) | DMA_CONFIG_NDSIZE(5) | DMA_CONFIG_FLOW(DMA_FLOW_DESCRIPTOR_LIST_LARGE);

    bfin_dma_channel_enable(DMA_CHANNEL_EMAC_RX, &rxDMADesc[0]);

    /* Enable Receiving, Automatic Pad Stripping and receiving of frames with
   * length < 64 butes */
