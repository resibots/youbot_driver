 ...
 * compensate. If needed the packets from interface A are resend through
 *interface B.
...
int ecx_setupnic(ecx_portt* port, const char* ifname, int secondary)
{
    ...
    *psock = pcap_open(ifname, 65536, PCAP_OPENFLAG_PROMISCUOUS | PCAP_OPENFLAG_MAX_RESPONSIVENESS | PCAP_OPENFLAG_NOCAPTURE_LOCAL,
        0, NULL, errbuf);
    if (NULL == *psock) {
...
int ecx_outframe_red(ecx_portt* port, int idx)
{
    ...
    if (port->redstate != ECT_RED_NONE) {
        ...
        pcap_sendpacket(port->redport->sockhandle, (u_char const*)&(port->txbuf2),
            port->txbuflength2);
...
/** Blocking redundant receive frame function. If redundant mode is not active
 *then
 * it skips the secondary stack and redundancy functions. In redundant mode it
 *waits
 * for both (primary and secondary) frames to come in. The result goes in an
 *decision
 * tree that decides, depending on the route of the packet and its possible
 *missing arrival,
...
static int ecx_waitinframe_red(ecx_portt* port, int idx, osal_timert* timer)
{
     ...
     */
        ...
        /* primary socket got nothing or primary frame, and secondary socket got
     * secondary frame */
        /* we need to resend TX packet */
        if (((primrx == 0) && (secrx == RX_SEC)) || ((primrx == RX_PRIM) && (secrx == RX_SEC))) {
            /* If both primary and secondary have partial connection retransmit the
       * primary received
       * frame over the secondary socket. The result from the secondary received
       * frame is a combined
