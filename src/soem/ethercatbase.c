 ...
 * @return always 0
 */
int ecx_setupdatagram(ecx_portt* port, void* frame, uint8 com, uint8 idx,
    uint16 ADP, uint16 ADO, uint16 length, void* data)
{
    ec_comt* datagramP;
    uint8* frameP;
...
/** Add EtherCAT datagram to a standard ethernet frame with existing
 *datagram(s).
 *
 * @param[in] port        = port context struct
 * @param[out] frame      = framebuffer
 ...
 */
int ecx_adddatagram(ecx_portt* port, void* frame, uint8 com, uint8 idx,
    boolean more, uint16 ADP, uint16 ADO, uint16 length,
    void* data)
{
    ec_comt* datagramP;
    uint8* frameP;
 ...
 */
int ecx_BWR(ecx_portt* port, uint16 ADP, uint16 ADO, uint16 length, void* data,
    int timeout)
{
    uint8 idx;
    int wkc;
    ...
    /* setup datagram */
    ecx_setupdatagram(port, &(port->txbuf[idx]), EC_CMD_BWR, idx, ADP, ADO,
        length, data);
 ...
 * @return Workcounter or EC_NOFRAME
 */
int ecx_BRD(ecx_portt* port, uint16 ADP, uint16 ADO, uint16 length, void* data,
    int timeout)
{
    uint8 idx;
    int wkc;
    ...
    idx = ecx_getindex(port);
    /* setup datagram */
    ecx_setupdatagram(port, &(port->txbuf[idx]), EC_CMD_BRD, idx, ADP, ADO,
        length, data);
    /* send data and wait for answer */
    wkc = ecx_srconfirm(port, idx, timeout);
    if (wkc > 0) {
 ...
 *
 * @param[in] port        = port context struct
 * @param[in]  ADP        = Address Position, each slave ++, slave that has 0
 *excecutes
 * @param[in]  ADO        = Address Offset, slave memory address
 * @param[in]  length     = length of databuffer
 * @param[out] data       = databuffer to put slave data in
 ...
 * @return Workcounter or EC_NOFRAME
 */
int ecx_APRD(ecx_portt* port, uint16 ADP, uint16 ADO, uint16 length, void* data,
    int timeout)
{
    int wkc;
    uint8 idx;
    ...
    idx = ecx_getindex(port);
    ecx_setupdatagram(port, &(port->txbuf[idx]), EC_CMD_APRD, idx, ADP, ADO,
        length, data);
    wkc = ecx_srconfirm(port, idx, timeout);
    if (wkc > 0) {
        memcpy(data, &(port->rxbuf[idx][EC_HEADERSIZE]), length);
 ...
 * @return Workcounter or EC_NOFRAME
 */
int ecx_ARMW(ecx_portt* port, uint16 ADP, uint16 ADO, uint16 length, void* data,
    int timeout)
{
    int wkc;
    uint8 idx;
    ...
    idx = ecx_getindex(port);
    ecx_setupdatagram(port, &(port->txbuf[idx]), EC_CMD_ARMW, idx, ADP, ADO,
        length, data);
    wkc = ecx_srconfirm(port, idx, timeout);
    if (wkc > 0) {
        memcpy(data, &(port->rxbuf[idx][EC_HEADERSIZE]), length);
 ...
 * @return Workcounter or EC_NOFRAME
 */
int ecx_FRMW(ecx_portt* port, uint16 ADP, uint16 ADO, uint16 length, void* data,
    int timeout)
{
    int wkc;
    uint8 idx;
    ...
    idx = ecx_getindex(port);
    ecx_setupdatagram(port, &(port->txbuf[idx]), EC_CMD_FRMW, idx, ADP, ADO,
        length, data);
    wkc = ecx_srconfirm(port, idx, timeout);
    if (wkc > 0) {
        memcpy(data, &(port->rxbuf[idx][EC_HEADERSIZE]), length);
 ...
 * @return Workcounter or EC_NOFRAME
 */
int ecx_FPRD(ecx_portt* port, uint16 ADP, uint16 ADO, uint16 length, void* data,
    int timeout)
{
    int wkc;
    uint8 idx;
    ...
    idx = ecx_getindex(port);
    ecx_setupdatagram(port, &(port->txbuf[idx]), EC_CMD_FPRD, idx, ADP, ADO,
        length, data);
    wkc = ecx_srconfirm(port, idx, timeout);
    if (wkc > 0) {
        memcpy(data, &(port->rxbuf[idx][EC_HEADERSIZE]), length);
 ...
 * @return Workcounter or EC_NOFRAME
 */
int ecx_APWR(ecx_portt* port, uint16 ADP, uint16 ADO, uint16 length, void* data,
    int timeout)
{
    uint8 idx;
    int wkc;
    ...
    idx = ecx_getindex(port);
    ecx_setupdatagram(port, &(port->txbuf[idx]), EC_CMD_APWR, idx, ADP, ADO,
        length, data);
    wkc = ecx_srconfirm(port, idx, timeout);
    ecx_setbufstat(port, idx, EC_BUF_EMPTY);
 ...
 * @return Workcounter or EC_NOFRAME
 */
int ecx_APWRw(ecx_portt* port, uint16 ADP, uint16 ADO, uint16 data,
    int timeout)
{
 ...
 * @return Workcounter or EC_NOFRAME
 */
int ecx_FPWR(ecx_portt* port, uint16 ADP, uint16 ADO, uint16 length, void* data,
    int timeout)
{
    int wkc;
    uint8 idx;
    ...
    idx = ecx_getindex(port);
    ecx_setupdatagram(port, &(port->txbuf[idx]), EC_CMD_FPWR, idx, ADP, ADO,
        length, data);
    wkc = ecx_srconfirm(port, idx, timeout);
    ecx_setbufstat(port, idx, EC_BUF_EMPTY);
 ...
 * @return Workcounter or EC_NOFRAME
 */
int ecx_FPWRw(ecx_portt* port, uint16 ADP, uint16 ADO, uint16 data,
    int timeout)
{
 ...
 * @return Workcounter or EC_NOFRAME
 */
int ecx_LRW(ecx_portt* port, uint32 LogAdr, uint16 length, void* data,
    int timeout)
{
    uint8 idx;
    int wkc;
    ...
    idx = ecx_getindex(port);
    ecx_setupdatagram(port, &(port->txbuf[idx]), EC_CMD_LRW, idx, LO_WORD(LogAdr),
        HI_WORD(LogAdr), length, data);
    wkc = ecx_srconfirm(port, idx, timeout);
 ...
 * @return Workcounter or EC_NOFRAME
 */
int ecx_LRD(ecx_portt* port, uint32 LogAdr, uint16 length, void* data,
    int timeout)
{
    uint8 idx;
    int wkc;
    ...
    idx = ecx_getindex(port);
    ecx_setupdatagram(port, &(port->txbuf[idx]), EC_CMD_LRD, idx, LO_WORD(LogAdr),
        HI_WORD(LogAdr), length, data);
    wkc = ecx_srconfirm(port, idx, timeout);
 ...
 * @return Workcounter or EC_NOFRAME
 */
int ecx_LWR(ecx_portt* port, uint32 LogAdr, uint16 length, void* data,
    int timeout)
{
    uint8 idx;
    int wkc;
    ...
    idx = ecx_getindex(port);
    ecx_setupdatagram(port, &(port->txbuf[idx]), EC_CMD_LWR, idx, LO_WORD(LogAdr),
        HI_WORD(LogAdr), length, data);
    wkc = ecx_srconfirm(port, idx, timeout);
    ecx_setbufstat(port, idx, EC_BUF_EMPTY);
 ...
 * @return Workcounter or EC_NOFRAME
 */
int ecx_LRWDC(ecx_portt* port, uint32 LogAdr, uint16 length, void* data,
    uint16 DCrs, int64* DCtime, int timeout)
{
    uint16 DCtO;
    uint8 idx;
    int wkc;
    ...
    DCtE = htoell(*DCtime);
    DCtO = ecx_adddatagram(port, &(port->txbuf[idx]), EC_CMD_FRMW, idx, FALSE,
        DCrs, ECT_REG_DCSYSTIME, sizeof(DCtime), &DCtE);
    wkc = ecx_srconfirm(port, idx, timeout);
    if ((wkc > 0) && (port->rxbuf[idx][EC_CMDOFFSET] == EC_CMD_LRW)) {
        memcpy(data, &(port->rxbuf[idx][EC_HEADERSIZE]), length);
...
#ifdef EC_VER1
int ec_setupdatagram(void* frame, uint8 com, uint8 idx, uint16 ADP, uint16 ADO,
    uint16 length, void* data)
{
    return ecx_setupdatagram(&ecx_port, frame, com, idx, ADP, ADO, length, data);
}

int ec_adddatagram(void* frame, uint8 com, uint8 idx, boolean more, uint16 ADP,
    uint16 ADO, uint16 length, void* data)
{
    return ecx_adddatagram(&ecx_port, frame, com, idx, more, ADP, ADO, length,
        data);
}
...
int ec_LRWDC(uint32 LogAdr, uint16 length, void* data, uint16 DCrs,
    int64* DCtime, int timeout)
{
