 ...
 * @param[in] usetable     = TRUE when using configtable to init slaves, FALSE
 *otherwise
...
int ecx_config_init(ecx_contextt* context, uint8 usetable)
{
    ...
    *(context->slavecount) = 0;
    ...
    /* make special pre-init register writes to enable MAC[1] local administered
   * bit *
   * setting for old netX100 slaves */
    b = 0x00;
    ecx_BWR(context->port, 0x0000, ECT_REG_DLALIAS, sizeof(b), &b,
        EC_TIMEOUTRET3); /* Ignore Alias register */
    b = EC_STATE_INIT | EC_STATE_ACK;
    ecx_BWR(context->port, 0x0000, ECT_REG_ALCTL, sizeof(b), &b,
        EC_TIMEOUTRET3); /* Reset all slaves to Init */
    /* netX100 should now be happy */

    wkc = ecx_BWR(context->port, 0x0000, ECT_REG_ALCTL, sizeof(b), &b,
        EC_TIMEOUTRET3); /* Reset all slaves to Init */
    printf("wkc = %d\n", wkc);

    w = 0x0000;
    wkc = ecx_BRD(context->port, 0x0000, ECT_REG_TYPE, sizeof(w), &w,
        EC_TIMEOUTSAFE); /* detect number of slaves */
    if (wkc > 0) {
        *(context->slavecount) = wkc;
        b = 0x00;
        ecx_BWR(context->port, 0x0000, ECT_REG_DLPORT, sizeof(b), &b,
            EC_TIMEOUTRET3); /* deact loop manual */
        w = htoes(0x0004);
        ecx_BWR(context->port, 0x0000, ECT_REG_IRQMASK, sizeof(w), &w,
            EC_TIMEOUTRET3); /* set IRQ mask */
        ecx_BWR(context->port, 0x0000, ECT_REG_RXERR, 8, &zbuf,
            EC_TIMEOUTRET3); /* reset CRC counters */
        ecx_BWR(context->port, 0x0000, ECT_REG_FMMU0, 16 * 3, &zbuf,
            EC_TIMEOUTRET3); /* reset FMMU's */
        ecx_BWR(context->port, 0x0000, ECT_REG_SM0, 8 * 4, &zbuf,
            EC_TIMEOUTRET3); /* reset SyncM */
        ecx_BWR(context->port, 0x0000, ECT_REG_DCSYSTIME, 4, &zbuf,
            EC_TIMEOUTRET3); /* reset system time+ofs */
        w = htoes(0x1000);
        ecx_BWR(context->port, 0x0000, ECT_REG_DCSPEEDCNT, sizeof(w), &w,
            EC_TIMEOUTRET3); /* DC speedstart */
        w = htoes(0x0c00);
        ecx_BWR(context->port, 0x0000, ECT_REG_DCTIMEFILT, sizeof(w), &w,
            EC_TIMEOUTRET3); /* DC filt expr */
        b = 0x00;
        ecx_BWR(context->port, 0x0000, ECT_REG_DLALIAS, sizeof(b), &b,
            EC_TIMEOUTRET3); /* Ignore Alias register */
        b = EC_STATE_INIT | EC_STATE_ACK;
        ecx_BWR(context->port, 0x0000, ECT_REG_ALCTL, sizeof(b), &b,
            EC_TIMEOUTRET3); /* Reset all slaves to Init */
        b = 2;
        ecx_BWR(context->port, 0x0000, ECT_REG_EEPCFG, sizeof(b), &b,
            EC_TIMEOUTRET3); /* force Eeprom from PDI */
        b = 0;
        ecx_BWR(context->port, 0x0000, ECT_REG_EEPCFG, sizeof(b), &b,
            EC_TIMEOUTRET3); /* set Eeprom to master */

        for (slave = 1; slave <= *(context->slavecount); slave++) {
            ADPh = (uint16)(1 - slave);
            context->slavelist[slave].Itype = etohs(ecx_APRDw(context->port, ADPh, ECT_REG_PDICTL,
                EC_TIMEOUTRET3)); /* read interface type of slave */
            /* a node offset is used to improve readibility of network frames */
            /* this has no impact on the number of addressable slaves (auto wrap
       * around) */
            ecx_APWRw(context->port, ADPh, ECT_REG_STADR,
                htoes(slave + EC_NODEOFFSET),
                EC_TIMEOUTRET3); /* set node address of slave */
            if (slave == 1) {
            ...
            ecx_APWRw(context->port, ADPh, ECT_REG_DLCTL, htoes(b),
                EC_TIMEOUTRET3); /* set non ecat frame behaviour */
            configadr = etohs(ecx_APRDw(context->port, ADPh, ECT_REG_STADR, EC_TIMEOUTRET3));
            context->slavelist[slave].configadr = configadr;
            ecx_FPRD(context->port, configadr, ECT_REG_ALIAS, sizeof(aliasadr),
                &aliasadr, EC_TIMEOUTRET3);
            context->slavelist[slave].aliasadr = etohs(aliasadr);
            ecx_FPRD(context->port, configadr, ECT_REG_EEPSTAT, sizeof(estat), &estat,
                EC_TIMEOUTRET3);
            estat = etohs(estat);
        ...
        for (slave = 1; slave <= *(context->slavecount); slave++) {
            context->slavelist[slave].eep_man = etohl(ecx_readeeprom2(context, slave, EC_TIMEOUTEEP)); /* Manuf */
            ecx_readeeprom1(context, slave, ECT_SII_ID); /* ID */
        }
        for (slave = 1; slave <= *(context->slavecount); slave++) {
            context->slavelist[slave].eep_id = etohl(ecx_readeeprom2(context, slave, EC_TIMEOUTEEP)); /* ID */
            ecx_readeeprom1(context, slave, ECT_SII_REV); /* revision */
        }
        for (slave = 1; slave <= *(context->slavecount); slave++) {
            context->slavelist[slave].eep_rev = etohl(ecx_readeeprom2(context, slave, EC_TIMEOUTEEP)); /* revision */
            ecx_readeeprom1(
                context, slave,
                ECT_SII_RXMBXADR); /* write mailbox address + mailboxsize */
        }
        for (slave = 1; slave <= *(context->slavecount); slave++) {
            eedat = etohl(ecx_readeeprom2(
                context, slave,
                EC_TIMEOUTEEP)); /* write mailbox address and mailboxsize */
            ...
            if (context->slavelist[slave].mbx_l > 0) {
                ecx_readeeprom1(context, slave,
                    ECT_SII_TXMBXADR); /* read mailbox offset */
            }
        }
        for (slave = 1; slave <= *(context->slavecount); slave++) {
            if (context->slavelist[slave].mbx_l > 0) {
                eedat = etohl(ecx_readeeprom2(context, slave,
                    EC_TIMEOUTEEP)); /* read mailbox offset */
            ...
            if ((etohs(ecx_FPRDw(context->port, configadr, ECT_REG_ESCSUP,
                     EC_TIMEOUTRET3)) & 0x04) > 0) /* Support DC? */
            {
            ...
            topology = etohs(
                ecx_FPRDw(context->port, configadr, ECT_REG_DLSTAT,
                    EC_TIMEOUTRET3)); /* extract topology from DL status */
            h = 0;
            b = 0;
            ...
            /* ptype = Physical type*/
            context->slavelist[slave].ptype = LO_BYTE(etohs(ecx_FPRDw(
                context->port, configadr, ECT_REG_PORTDES, EC_TIMEOUTRET3)));
            ...
            w = ecx_statecheck(context, slave, EC_STATE_INIT,
                EC_TIMEOUTSTATE); //* check state change Init */
            ...
            if (context->slavelist[slave].mbx_l > 0) {
                ...
                context->slavelist[slave].mbx_proto = ecx_readeeprom(context, slave, ECT_SII_MBXPROTO, EC_TIMEOUTEEP);
            }
            cindex = 0;
#ifdef EC_VER1
            /* use configuration table ? */
            if (usetable) {
                cindex = ec_findconfig(context->slavelist[slave].eep_man,
...
#endif
            {
                ...
                if (nSM > 0) {
                    ...
                    context->slavelist[slave].SM[0].SMflags = htoel((context->eepSM->Creg) + (context->eepSM->Activate << 16));
                    SMc = 1;
                    while ((SMc < EC_MAXSM) && ecx_siiSMnext(context, slave, context->eepSM, SMc)) {
                        ...
                        context->slavelist[slave].SM[SMc].SMflags = htoel(
                            (context->eepSM->Creg) + (context->eepSM->Activate << 16));
                        SMc++;
                    }
                }
                /* SII FMMU section */
            ...
            if (context->slavelist[slave].mbx_l > 0) {
                ...
                {
                    EC_PRINT("Slave %d has no proper mailbox out configuration, try "
                             "default.\n",
                        slave);
    ...
    if ((*(context->slavecount) > 0) && (group < context->maxgroup)) {
        ...
        for (slave = 1; slave <= *(context->slavecount); slave++) {
            ...
            ecx_statecheck(context, slave, EC_STATE_PRE_OP,
                EC_TIMEOUTSTATE); /* check state change pre-op */
            ...
            if (!group || (group == context->slavelist[slave].group)) {
                ...
                if (!context->slavelist[slave].mbx_l && context->slavelist[slave].SM[0].StartAddr) {
                    ecx_FPWR(context->port, configadr, ECT_REG_SM0, sizeof(ec_smt),
                        &(context->slavelist[slave].SM[0]), EC_TIMEOUTRET3);
                ...
                if (!context->slavelist[slave].mbx_l && context->slavelist[slave].SM[1].StartAddr) {
                    ecx_FPWR(context->port, configadr, ECT_REG_SM1, sizeof(ec_smt),
                        &context->slavelist[slave].SM[1], EC_TIMEOUTRET3);
                ...
                for (nSM = 2; nSM < EC_MAXSM; nSM++) {
                    if (context->slavelist[slave].SM[nSM].StartAddr) {
                        ...
                        if (context->slavelist[slave].SM[nSM].SMlength == 0) {
                            context->slavelist[slave].SM[nSM].SMflags = htoel(etohl(context->slavelist[slave].SM[nSM].SMflags) & EC_SMENABLEMASK);
                        }
                        ecx_FPWR(context->port, configadr,
                            ECT_REG_SM0 + (nSM * sizeof(ec_smt)), sizeof(ec_smt),
                            &context->slavelist[slave].SM[nSM], EC_TIMEOUTRET3);
                ...
                if (context->slavelist[slave].Obits) {
                    ...
                    while ((SMc < (EC_MAXSM - 1)) && (FMMUdone < ((context->slavelist[slave].Obits + 7) / 8))) {
                        ...
                        while ((BitCount < context->slavelist[slave].Obits) && (SMc < (EC_MAXSM - 1))) /* more SM for output */
                        {
                            ...
                            while ((SMc < (EC_MAXSM - 1)) && (context->slavelist[slave].SMtype[SMc] != 3))
                                SMc++;
                            /* if addresses from more SM connect use one FMMU otherwise break
               * up in mutiple FMMU */
                        ...
                        ecx_FPWR(context->port, configadr,
                            ...
                            sizeof(ec_fmmut), &(context->slavelist[slave].FMMU[FMMUc]),
                            EC_TIMEOUTRET3);
                        context->grouplist[group].outputsWKC++;
                        if (!context->slavelist[slave].outputs) {
                            context->slavelist[slave].outputs = (uint8*)(pIOmap) + etohl(context->slavelist[slave].FMMU[FMMUc].LogStart);
                            context->slavelist[slave].Ostartbit = context->slavelist[slave].FMMU[FMMUc].LogStartbit;
            ...
            if (context->slavelist[slave].Ibits) {
                ...
                while ((SMc < (EC_MAXSM - 1)) && (FMMUdone < ((context->slavelist[slave].Ibits + 7) / 8))) {
                    ...
                    while ((BitCount < context->slavelist[slave].Ibits) && (SMc < (EC_MAXSM - 1))) /* more SM for input */
                    {
                        ...
                        while ((SMc < (EC_MAXSM - 1)) && (context->slavelist[slave].SMtype[SMc] != 4))
                            SMc++;
                        /* if addresses from more SM connect use one FMMU otherwise break up
             * in mutiple FMMU */
                        if (etohs(context->slavelist[slave].SM[SMc].StartAddr) > EndAddr) {
                            break;
                        }
                    ...
                    if (!context->slavelist[slave].inputs) {
                        context->slavelist[slave].inputs = (uint8*)(pIOmap) + etohl(context->slavelist[slave].FMMU[FMMUc].LogStart);
                        context->slavelist[slave].Istartbit = context->slavelist[slave].FMMU[FMMUc].LogStartbit;
...
int ecx_recover_slave(ecx_contextt* context, uint16 slave, int timeout)
{
    ...
    readadr = 0xfffe;
    wkc = ecx_APRD(context->port, ADPh, ECT_REG_STADR, sizeof(readadr), &readadr,
        timeout);
    ...
    if ((wkc > 0) && (readadr == 0)) {
        ...
        if (ecx_APWRw(context->port, ADPh, ECT_REG_STADR, htoes(EC_TEMPNODE),
                timeout) <= 0) {
            ecx_FPWRw(context->port, EC_TEMPNODE, ECT_REG_STADR, htoes(0), 0);
...
int ecx_reconfig_slave(ecx_contextt* context, uint16 slave, int timeout)
{
    ...
    configadr = context->slavelist[slave].configadr;
    if (ecx_FPWRw(context->port, configadr, ECT_REG_ALCTL, htoes(EC_STATE_INIT),
            timeout) <= 0) {
        return 0;
    }
    state = 0;
    ...
    if (state == EC_STATE_INIT) {
        ...
        ecx_FPWRw(context->port, configadr, ECT_REG_ALCTL, htoes(EC_STATE_PRE_OP),
            timeout);
        state = ecx_statecheck(context, slave, EC_STATE_PRE_OP,
            EC_TIMEOUTSTATE); /* check state change pre-op */
        if (state == EC_STATE_PRE_OP) {
            ...
            ecx_FPWRw(context->port, configadr, ECT_REG_ALCTL,
                htoes(EC_STATE_SAFE_OP), timeout); /* set safeop status */
            state = ecx_statecheck(context, slave, EC_STATE_SAFE_OP,
                EC_TIMEOUTSTATE); /* check state change safe-op */
            /* program configured FMMU */
            for (FMMUc = 0; FMMUc < context->slavelist[slave].FMMUunused; FMMUc++) {
                ecx_FPWR(context->port, configadr,
                    ECT_REG_FMMU0 + (sizeof(ec_fmmut) * FMMUc), sizeof(ec_fmmut),
                    &context->slavelist[slave].FMMU[FMMUc], timeout);
            }
        }
    }
 ...
 * @return IOmap size
 */
int ec_config_map(void* pIOmap) { return ec_config_map_group(pIOmap, 0); }
