...
void YouBotBaseTestWithoutThread::setUp()
{
    ...
    if (ethercatMaster->isThreadActive()) {
        ...
        ethercatMaster = &EthercatMaster::getInstance("youbot-ethercat.cfg",
                             "../config/", false);
    }
...
void YouBotBaseTestWithoutThread::YouBotBaseTestWithoutThread_PositionMode()
{
    ...
    while (myTrace.getTimeDurationMilliSec() < overallTime) {
        ...
        if (myTrace.getTimeDurationMilliSec() > startTime + durationNull && myTrace.getTimeDurationMilliSec() < startTime + durationNull + stepStartTime) {
